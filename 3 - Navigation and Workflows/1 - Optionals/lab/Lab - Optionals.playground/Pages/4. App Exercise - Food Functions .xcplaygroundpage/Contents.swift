...
func getMeal(command: String) -> Meal? {
    if let theMeal = meals[command] {
        return theMeal
    } else {
        return nil
    }
 ...
 */
func getDefault(key: String) -> [String: Any]? {
    if let defaultValue = UserDefaults.standard.dictionary(forKey: key) {
        return defaultValue
    } else {
        return [String: Any]()
    }
