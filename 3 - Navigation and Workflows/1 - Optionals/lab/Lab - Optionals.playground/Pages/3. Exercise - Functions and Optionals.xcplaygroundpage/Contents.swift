...
func getPrice(name: String) -> Double? {
    if let theStock = stock[name] {
        if theStock > 0 {
            if let thePrice = prices[name] {
                return thePrice
            } else {
                return nil
            }
...
print(getPrice(name: "algo"))
print(getPrice(name: "Juice"))
print(getPrice(name: "Donuts"))
