...
func add(x: Int?, y: Int?) -> Int? {
    guard let num1 = x, let num2 = y else { return nil }
    return num1 + num2
}
...
func createUser() -> User? {
    guard let fn = firstNameTextField.text, let ln = lastNameTextField.text, let age = ageTextField.text else { return nil }
    return User(firstName: fn, lastName: ln, age: age)
}
/*:
 ...
 */
if let u1 = createUser() {
    print("\(u1.firstName), \(u1.lastName), \(u1.age)")
}
